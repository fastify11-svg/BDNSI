                                    ...
                                    {templates.length === 0 ? (
                                        <tr>
                                            <td colSpan="5" className="px-6 py-12 whitespace-nowrap text-center">
                                                ...
                                                <div className="mt-6">
                                                    <Link href={getUrl('/admin/document-templates/create')} className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                                        Create Template
                                                    </Link>
                                                </div>
                                            </td>
                                        ...
                                        templates.map((template) => (
                                            <tr key={template.id} className="hover:bg-gray-50/50 transition-colors duration-200">
                                                    ...
                                                    <div className="flex items-center">
                                                        ...
                                                        <div className="ml-4">
                                                            <div className="text-sm font-semibold text-gray-900 flex items-center">
                                                                {template.name}
                                                                {template.is_builtin && <span className="ml-2 px-1.5 py-0.5 inline-flex text-[10px] leading-4 font-semibold rounded bg-amber-100 text-amber-800">Built-in</span>}
                                                            </div>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800 capitalize shadow-sm">{template.type.replace('_', ' ')}</span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-medium">{template.width} <span className="text-gray-400 font-normal mx-1">×</span> {template.height}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                                    ...
                                                    >
                                                        ...
                                                        <span aria-hidden="true" className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${template.status ? 'translate-x-5' : 'translate-x-0'}`} />
                                                    </button>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div className="flex items-center justify-end space-x-3">
                                                        <a href={getUrl(`/admin/document-templates/${template.id}/preview`)} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-indigo-600 flex items-center transition-colors tooltip" title="Live Preview">
                                                        ...
                                                        {template.is_builtin ? (
                                                            <span className="inline-flex items-center px-3 py-1.5 border border-gray-200 text-xs font-medium rounded text-gray-500 bg-gray-50 cursor-not-allowed" title="Built-in templates cannot be edited visually">
                                                                <svg className="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                                                                Locked
                                                            </span>
                                                        ) : (
                                                            <a
